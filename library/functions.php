...
function contactUs($username,$email,$message,$site_email)
{
 ...
 $site_email = secureInput($site_email);
		...
		$message = "
			<html>
			  <head>
				<title>Новое сообщение от".$username."</title>
			  </head>
			  <body>
				<h3>Вопрос / Предложение</h3>
				<p>Пользователь, ".$username." прислал сообщение. Вот его содержание:</p>
				<p>".$message."</p>
			  </body>
			</html>
			";
...
//----------Function for adding user's review----------
function addReview($user,$rating,$email,$rvtext,$pdid,$usrid)
{
	$user = secureInput($user);
	$email = secureInput($email);
	$rvtext = secureInput($rvtext);

	$sql = "INSERT INTO tbl_review (rv_pd_id,rv_usr_id,rv_usr_name,rv_usr_email,rv_usr_rating,rv_text,rv_date) VALUES ('".$pdid."','".$usrid."','".$user."','".$email."','".$rating."','".$rvtext."',NOW())";
	$res = mysql_query($sql) or die(mysql_error());
	if($res){return 99;} else {return 1;}
}
